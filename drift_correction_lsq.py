...
def solve_line(group):
    ...
    for j, loop_id in enumerate(loops):
        loop_rows.append({
            ...
            "offset_uGal":     s_vals[j] * 1000,
            "SE_offset_uGal":  se_s[j] * 1000,
        })
...
def main(config_name="decay"):
    ...
    out_file = DATA_DIR / f"lsq_corrected_{config_name}.csv"
    ...
    for _, row in counts.iterrows():
        ...
        res_uGal = grp["residual"].abs() * 1000
        print(f"  Line {int(row['Line'])} loc {int(row['loc_id'])}: "
              f"{labels}  |resid| = "
              f"{', '.join(f'{v:.2f}' for v in res_uGal)} µGal")
