...
ALSO A LIBRARY: plot_petal_migration_3d imports the migration-taper helper and
padding/taper constants from here -- `live_sample_taper`, `PAD_T_FACTOR`,
`PAD_X_TRACES`, `TAPER_W`, `TAPER_T_FRAC` -- to migrate the petal segments with the
same machinery. Change those signatures/values with care.
"""
...
# Compute core lives in migrate_scan_io (F11 split). Re-import the helpers +
# padding/taper constants here so plot_petal_migration_3d's existing
# `from migrate_velocity_scan import live_sample_taper, PAD_*` keeps working.
from migrate_scan_io import (
    live_sample_taper, tgain_weights, norm99,
    migrate_at_velocity, save_migrated_npz,
    PAD_T_FACTOR, PAD_X_TRACES, TAPER_W, TAPER_T_FRAC,
    LIVE_TAPER_X, LIVE_TAPER_T)
...
def main():
    ...
    if args.pick_velocity is not None:
        ...
        mig, section, dead, depth = migrate_at_velocity(
            data0, x, t, elevations, v, no_live_taper=args.no_live_taper)

        # --- save NPZ ---
        out_npz = save_migrated_npz(
            HERE / '../../Data/GPR/Migration' / (args.line + '_migrated.npz'),
            mig, x, depth, ref_elev, elevations, v)
    ...
    for v in vels:
        # topo-corrected at THIS velocity, then Stolt-migrated (migrate_scan_io)
        mig, section, dead, depth = migrate_at_velocity(
            data0, x, t, elevations, float(v), no_live_taper=args.no_live_taper)
