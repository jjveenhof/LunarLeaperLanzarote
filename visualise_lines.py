...
CORR_FILE = BASE / "Data/Gravimetry/drift_corrected.csv"
...
def along_profile_distance(df):
    ...
    # Principal axis via 2×2 covariance eigen-decomposition
...
def plot_line(ax, line_df, line_id):
    ...
    for loop_id in loop_ids:
        ...
        for stype, grp in loop.groupby("StationType"):
            grp = grp[grp["dist"].notna()]
            if grp.empty:
                continue
            ax.errorbar(
    ...
    # Transition base stations (not in any loop)
    orphans = plot_df[plot_df["loop_id"].isna() & plot_df["Grav_corr"].notna()]
    for stype, grp in orphans.groupby("StationType"):
        grp = grp[grp["dist"].notna()]
        if grp.empty:
            continue
        ax.errorbar(
